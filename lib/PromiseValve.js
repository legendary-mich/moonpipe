...
class PromiseValve extends BaseValve {
  ...
  constructor(preset, promiseFactory) {
    ...
    this.repeatVerbose = preset.repeatVerbose,
    ...
    try {
      ...
      while (true) { // eslint-disable-line no-constant-condition
        try {
          ...
          if (this.repeatVerbose) this.onError(err)
...
function validatePreset(preset, promiseFactory) {
  ...
  if (preset.repeatVerbose && typeof preset.repeatVerbose !== 'boolean') {
    throw new Error(`Unexpected 'repeatVerbose': ${preset.repeatVerbose}`)
  }

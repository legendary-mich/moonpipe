  ...
  async run(value, repeatCounter) {
    return new Promise(async (__resolve, __reject) => {
      ...
      const reject = async (error) => {
        ...
        if (this.isCanceled) return
        try {
          // NOTE: The timeout does not apply to the
          // repeatPredicate. If the repeatPredicate promise hangs,
          // there's nothing that can cancel it.
          if (repeatCounter > 0 && await this.repeatPredicate(error)) {
            _resolve(this.run(value, repeatCounter - 1))
          }
          else {
            _reject(error)
          }
          ...
          // repeatCounter and the condition in the predicate.
          _reject(err)
        }
      }

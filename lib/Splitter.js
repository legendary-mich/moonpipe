  ...
  next() {
    this.dataBuckets.forEach((bucket, key) => {
      ...
      if (!mp && this.idlePipes.length > 0) {
        mp = this.assignedPipes.get(key)
        if (mp) {
          this.idlePipes = this.idlePipes.filter(p => p !== mp)
        }
        else {
          mp = this.idlePipes.shift() // use the `shift` for round-robin
          // Pipes are assigned to a key in perpetuity in order to be
          // able to use the same cache across their lifetimes.
          this.assignedPipes.set(key, mp)
        }

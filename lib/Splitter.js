...
class Splitter {
  ...
  constructor(poolSize, classifyFn) {
    if (typeof poolSize !== 'number' || poolSize < 1) {
      throw new Error(`Expected poolSize to be a 'number' greater or equal to 1; found: ${poolSize}`)
    }
  ...
  clone() {
    return new Splitter(this.poolSize, this.classify)
  }
  ...
  cacheClearOne(valveIndex, ...values) {
    for (const mp of this.allPipes) {
      mp.cacheClearOne(valveIndex, ...values)
    }
  }
}

...
const { BasePresets } = require('./BaseValve.js')
...
class MudPipe {
  ...
  sliceEager(chunkSize, intervalMs) {
    const preset = Object.assign({
      maxBufferSize: chunkSize,
    }, TimePresets.sliceEager)
    const valve = new TimeValve(preset, intervalMs)
    return this.pipe(valve)
  }

  sliceLazy(chunkSize, intervalMs) {
    const preset = Object.assign({
      maxBufferSize: chunkSize,
    }, TimePresets.sliceLazy)
    const valve = new TimeValve(preset, intervalMs)
    return this.pipe(valve)
  }
  ...
  filter(transformFunc, options) {
    const preset = Object.assign({}, BasePresets.queue, options)
    const valve = new FilterValve(preset, transformFunc)
    return this.pipe(valve, transformFunc)
  }
}

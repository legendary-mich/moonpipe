...
class MoonPipe {
  ...
  constructor() {
    ...
    // The splittersStack is used only during the piping phase. It
    // will be empty again once you have called the join() method as
    // many times as you have called the splitBy() method.
    this.splittersStack = []
  }
   ...
   */
  get gatewaySplitter() {
    return this.splittersStack[0] || null
  }
   ...
   */
  isBusy() {
    return this.hooks.busyIdleLatch.isUp
  }
  ...
  pipe(valve, inputChannel = CHANNEL_TYPE.DATA, outputChannel = CHANNEL_TYPE.DATA) {
    ...
    if (this.gatewaySplitter) {
      ...
      valve.onReady = () => {
        if (valve.isIdle) this.channelCleanUp()
        else this.next(valveIndex)
      }
    ...
    // on the gatewaySplitter, and only then push the new Splitter on
    // the stack.
    if (valve instanceof Splitter) {
      this.splittersStack.push(valve)
    }
    return this
  }
  ...
  _pump(data, index = 0, channel = CHANNEL_TYPE.DATA) {
    ...
    if (!flushed) {
      ...
      if (channel === CHANNEL_TYPE.ERROR && this.hooks.onError) {
        // The onError callback could be made public and documented in the
        // README file. However, it should be clearly stated that the pipe does
        // not pause after emitting an error via onError, and that generally
        // errors are better handled with error valves. Currently I see no use
        // for the onError callback, other than the internal use in Splitters.
  ...
  pump(data) {
    ...
    // Normally, channelCleanUp is run from the onReady hook of one of the
    // valves. Cases in which not a single onReady hook is called, making me to
    // have to call the channelCleanUp here, are:
    // 1. When there are no valves attached.
    // 2. When there are no error valves attached, and an error is emitted
    // from the onBusy hook.
    this.channelCleanUp()
  }
    ...
    if (this.hooks.busyIdleLatch.tryDown()) {
      if (this.hooks.onIdle) {
        try {
          ...
          // The _pump method does not flip the busyIdleLatch. Therefore, after
          // running the _pump method, the pipe is still in a notBusy state, so
          // there is no danger of calling the onIdle hook recursively.
          this._pump(err, 0, CHANNEL_TYPE.ERROR)
        }
      }
      if (this.hooks.resolve) {
   ...
   */
  onBusyBy(callback) {
    this.validateHookCallback('onBusyBy', callback)
    const splitter = this.splittersStack.at(-1)
    splitter.hooks.onBusyBy = callback
    return this
  }
   ...
   */
  onIdleBy(callback) {
    this.validateHookCallback('onIdleBy', callback)
    const splitter = this.splittersStack.at(-1)
    splitter.hooks.onIdleBy = callback
    return this
  }
   ...
   */
  join() {
    if (this.splittersStack.length < 1) {
      throw new Error('There are no splitters to join')
    }
    // The order is important here. First pop a Splitter from the
    // stack and only then try to join the gatewaySplitter. If the
    // gatewaySplitter is the last splitter on the stack, all its
    // children has already been joined so it is enough to remove it
    // from the stack.
    this.splittersStack.pop()
  ...
  validateHookCallback(hookName, callback) {
    ...
    if (['onBusyBy', 'onIdleBy'].includes(hookName)) {
      if (this.splittersStack.length === 0) {
        throw new Error('There are no Splitters on the stack')
      }
      const splitter = this.splittersStack.at(-1)
      if (splitter.hooks[hookName]) {
